...
func main() {
	sconfig := oauth.NewServerConfig()
	sconfig.AllowedAuthorizeTypes = oauth.AllowedAuthorizeType{oauth.CODE, oauth.TOKEN}
	sconfig.AllowedAccessTypes = oauth.AllowedAccessType{oauth.AUTHORIZATION_CODE,
		oauth.REFRESH_TOKEN, oauth.PASSWORD, oauth.CLIENT_CREDENTIALS, oauth.ASSERTION}
	sconfig.AllowGetAccessRequest = true
	sconfig.AllowClientSecretInParams = true
	server := oauth.NewServer(sconfig, example.NewTestStorage())
	...
	http.HandleFunc("/token", func(w http.ResponseWriter, r *http.Request) {
		...
		if ar := server.HandleAccessRequest(resp, r); ar != nil {
			...
			case oauth.CLIENT_CREDENTIALS:
				ar.Authorized = true
			case oauth.ASSERTION:
				if ar.AssertionType == "urn:oauth.example.complete" && ar.Assertion == "oauth.data" {
					ar.Authorized = true
				}
			}
	...
	http.HandleFunc("/appauth/assertion", func(w http.ResponseWriter, r *http.Request) {
		...
		// build access code url
		aurl := fmt.Sprintf("/token?grant_type=assertion&assertion_type=urn:oauth.example.complete&assertion=oauth.data")
